...
public final class Message implements Parcelable {
    ...
    public static Message obtain() {
        synchronized (sPoolSync) {
            if (sPool != null) {
                // 取出消息池中的头结点
                // 将sPool往后移动一位, 指向消息池中的第二个结点
                // 让头结点的尾指针不再指向第二个结点, 这样这个message就和消息池脱离了
                // 从此消息池中的第二个结点成了消息池中的头结点, sPool还是指向消息池的头结点
                Message m = sPool;
                sPool = m.next;
                m.next = null;
    ...
    void recycleUnchecked() {
        ...
        synchronized (sPoolSync) {
            if (sPoolSize < MAX_POOL_SIZE) {
                // 将这个消息和消息池的头结点进行关联, 使这个消息的尾指针指向消息池的头结点
                // 这样这个消息就成了消息池新的头结点了
                // 然后将sPool指向消息池的头结点, 也就是该消息本身
                // 其实sPool指向的就是消息池的头结点
                next = sPool;
                sPool = this;
                sPoolSize++;
